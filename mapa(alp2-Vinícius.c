...
arq = fopen("Dados.txt", "a+");      //arquivo de texto automático para gerar a ficha dos dados do paciente
fprintf(arq,"\t\t DADOS DO PACIENTE: \n");
fprintf(arq,"Paciente de nome: %s \n",nome);
fprintf(arq,"sexo : %s\n",sexo);
fprintf(arq,"cpf: %d\n",cpf);
fprintf(arq,"idade: %d\n",idade);
fprintf(arq,"A pontuação de sintomas foi de: %d\n",vl);
fclose(arq);
